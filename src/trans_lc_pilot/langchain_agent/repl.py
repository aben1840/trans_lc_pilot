"""Interactive REPL for the LangChain agent.

Every line is sent to the agent as a prompt. Commands (``/help``,
``/quit``) are the only non-prompt inputs. The agent is built lazily
on the first prompt so the REPL can start up without an API key.
"""
...
def run(
    ...
    *,
    ...
    print("trans-lc-pilot-agent REPL. Ctrl-D to exit.")
