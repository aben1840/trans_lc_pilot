...
**审查要求：** 每次提交前必须经过人工审查和明确批准——不允许无人值守或自动提交。Agent 和 bot 可以准备提交（暂存文件、起草消息、展示 diff），但**只有当人类在当前回合明确要求时**（如"请提交当前变更"、"commit these changes"）才可以执行 `git commit`。否则，交由审查者手动执行 `git commit` / merge。
